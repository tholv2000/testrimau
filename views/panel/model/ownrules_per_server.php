	...
	<div class="page-content">						
          ...
          <div class="row">   
              ...
              <div class="col-md-12">
					...
					<div class="form-group">
						<label class="col-sm-3 control-label " for="form-field-1-1"> Rules File Name</label>

						<div class="col-sm-9">
							<input type="text" id="name" name="rules" placeholder="Rules File Name" class="form-control" />
						</div>
					</div>
                	<div class="form-group">
						...
						<div class="col-sm-9">
						<textarea class="form-control" id="rules" name="rules" rows="20"></textarea>	
						</div>
          ...
          <div class="row">
              ...
              <tr>
                  <th>Rules ID</th>
                  <th width="50%">Rules File  Name</th>
                  <th>Host</th>
                  <th>Function</th>
              </tr>
              </thead>
...
//	e.preventDefault()
	...
	if ($('#rules').val() === ''){
		...
		$.post('myrules/ownlist',{name:$('#name').val(),rules:$('#rules').val(),host:"<?=$host?>"},function(data){
		
			var link = $('li.active a[data-toggle="tab"]');
			link.parent().removeClass('active');
			var tabLink = link.attr('href');
			
			$('#paparx').load('panel/ownrules_per_server',{host:"<?=$host?>"}).show();
		}); 
	}
...
function padamrules(id){
	...
	if(confirm("Are you sure you want to delete this?")){
		...
		$.post('panel/padamownrules',{id:id,host:"<?=$host?>"},function(data){
			console.log(data);	
			$('#paparx').load('panel/ownrules_per_server',{host:"<?=$host?>"}).show();
