...
$("#addurl").click(function(){
	...
	if ($('#durl').val() === ''){
		...
		$.post('myrules/blacklist',{url:$('#durl').val(),jenis:0,host:"<?=$host?>"},function(data){
		
			var link = $('li.active a[data-toggle="tab"]');
			link.parent().removeClass('active');
			var tabLink = link.attr('href');
			
			$('#paparx').load('panel/black_per_server',{stab:tabLink,host:"<?=$host?>"}).show();
		}); 
	}
...
$("#addip").click(function(){
	...
	if ($('#dip').val() === ''){
		...
		$.post('myrules/blacklist',{url:$('#dip').val(),jenis:1,host:"<?=$host?>"},function(data){
...
function padamrules(id){
	...
	if(confirm("Are you sure you want to delete this?")){
		...
		$.post('panel/padamrules',{id:id,jenis:1,host:"<?=$host?>"},function(data){
		
		//var link = $('li.active a[data-toggle="tab"]');
                                //link.parent().removeClass('active');
                                //var tabLink = link.attr('href');

                                $('#paparx').load('panel/black_per_server',{host:"<?=$host?>"}).show();
