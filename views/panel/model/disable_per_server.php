...
$("#addidx").click(function(){
	...
	if ($('#ruleid').val() === '') {
		...
		$.post('myrules/disable_per_server',{url:$('#ruleid').val(),status:0,host:"<?=$host?>"},function(data){
		
			var link = $('li.active a[data-toggle="tab"]');
			link.parent().removeClass('active');
			var tabLink = link.attr('href');
			
			$('#paparx').load('panel/disablerules_per_server',{stab:tabLink,host:"<?=$host?>"}).show();
		}); 
	}
...
function padamrules(id){
	...
	if(confirm("Are you sure you want to delete this?")){
		...
		$.post('panel/padamrules_per_server',{id:id,jenis:3,host:"<?=$host?>"},function(data){
		
		$('#paparx').load('panel/disablerules_per_server',{host:"<?=$host?>"}).show();
...
function editrules(id){
	...
	$.post('panel/editrules_per_server',{id:id,jenis:3},function(data){
...
function saveedit(){
	...
	$.post('panel/editrulessimpan_per_server',dataserver,function(data){
